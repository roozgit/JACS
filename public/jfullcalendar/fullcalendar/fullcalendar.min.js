...
var defaults = {
	...
	monthNames: ['January','February','March','April','May','June','July','August','September','October','November','December'],
        jmonthNames: ['فروردین','اردیبهشت','خرداد','تیر','مرداد','شهریور','مهر','ابان','آذر','دی','بهمن','اسفند'],
	monthNamesShort: ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'],
        jmonthNamesShort: ['فروردین','اردیبهشت','خرداد','تیر','مرداد','شهریور','مهر','آبان','آذر','دی','بهمن','اسفند'],
	dayNames: ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'],
        jdayNames: ['یکشنبه','دوشنبه','سه شنبه','چهارشنبه','پنجشنبه','جمعه','شنبه'],
	dayNamesShort: ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'],
        jdayNamesShort: ['ی','د','س','چ','پ','ج','ش'],	
	buttonText: {
...
$.fn.fullCalendar = function(options) {
	...
	if(options.isJalali){
	  options.isRTL=true;
	  options.firstDay=6;
	  options.dayNames=defaults.jdayNames;
	  options.dayNamesShort=defaults.jdayNamesShort;
	  options.monthNames=defaults.jmonthNames;
	  options.monthNamesShort=defaults.jmonthNamesShort;	  
	  options.buttonText.today='امروز';
	  options.buttonText.week='هفته';
	  options.buttonText.month='ماه';
	  options.buttonText.day='روز';
	}
	//console.log(options);
		...
		if (elementVisible()) {
			...
			if (!currentView.start || inc || date < currentView.start || date >= currentView.end) {
				...
				//currentView.render(date, inc || 0); // responsible for clearing events
                                currentView.render(date, inc || 0,options.isJalali); // responsible for clearing events
...
function addJalaliMonths(d, n, keepTime) { // prevents day overflow/underflow    
	if (+d) { // prevent infinite looping on invalid dates            
		var m = d.getJalaliMonth() + n;                                        
                d.setJalaliMonth(m);		
                if(m<0){
                    addYears(d,-1,keepTime);                    
                    d.setJalaliMonth(11);	                    
                }                                  
            }
	return d;
...
function formatDates(date1, date2, format, options) {
	...
	for (i=0; i<len; i++) {
		...
		if (c == "'") {
			...
			for (i2=len; i2>i; i2--) {
                            if(options.isJalali){
                                if (formatter = jdateFormatters[format.substring(i, i2)]) {
					if (date) {
...
var jdateFormatters = {
	...
	ddd	: function(d,o)	{ return o.jdayNamesShort[d.getJalaliDay()] },
	dddd: function(d,o)	{ return o.jdayNames[d.getJalaliDay()] },
	M	: function(d)	{ return d.getJalaliMonth() + 1 },
	MM	: function(d)	{ return zeroPad(d.getJalaliMonth() + 1) },
	MMM	: function(d,o)	{ return o.jmonthNamesShort[d.getJalaliMonth()] },
	MMMM: function(d,o)	{ return o.jmonthNames[d.getJalaliMonth()] },
...
function MonthView(element, calendar) {
	...
	function render(date, delta,isJalali) {                        
		if (delta) {
                    if(isJalali){
                        addJalaliMonths(date, delta);
		...
		//start.setDate(1);
                if(isJalali){
                    ...
                    var end = addJalaliMonths(cloneDate(start), 1);
                    var visEnd = new Date();
                    visEnd.setTime(end.getTime());                    
                }else{
		start.setDate(1);
		...
		t.visStart = visStart;
		t.visEnd = visEnd;
		renderBasic(rowCnt, nwe ? 5 : 7, true,isJalali);
	}
...
function BasicView(element, calendar, viewName) {
	...
	function renderBasic(r, c, showNumbers,isJalali) {
		rowCnt = r;
		colCnt = c;
		updateOptions();
		var firstTime = !body;
		...
		buildTable(showNumbers,isJalali);
	}
	...
	function buildTable(showNumbers,isJalaly) {
		var html = '';
		var i, j;
		var headerClass = tm + "-widget-header";
		var contentClass = tm + "-widget-content";
		var month = (isJalaly)?t.start.getJalaliMonth():t.start.getMonth();
			...
			for (j=0; j<colCnt; j++) {
				...
				cellClasses = [
					...
					contentClass
                                ...
                                if(isJalaly){                                    
                                    if( cellDate.getJalaliMonth() != month){
				...
				if (showNumbers) {					
                                    if(isJalaly){
                                        html += "<div class='fc-day-number'>" + cellDate.getJalaliDate() + "</div>";
                                    }else{
